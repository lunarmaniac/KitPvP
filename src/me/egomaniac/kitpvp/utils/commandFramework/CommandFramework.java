package me.egomaniac.kitpvp.utils.commandFramework;
		...
		for (int i = args.length; i >= 0; i--) {
			...
			if (commandMap.containsKey(cmdLabel)) {
				...
				if (!command.permission().equals("") && !sender.hasPermission(command.permission())) {
					sender.sendMessage(CC.translate("&cYou don't have permission to execute this command.")); // changed from noperm()
					//sender.sendMessage(ChatHelper.translate(Main.getInstance().getConfig().getString("No-Permissions-Message"))); // changed from noperm()
					return true;
				}
				if (command.inGameOnly() && !(sender instanceof Player)) {
					sender.sendMessage(CC.translate("&cConsole may not use this command.")); // changed from default
					return true;
				}
